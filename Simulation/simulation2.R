...
simulation2 <- function(test_person, start_ages, baseline_spending, n){
  ...
  for(time in 1:n){
    ...
    for(k in 1:n){
      if(k == 1){
        spend_no[k] <- baseline_spending + spend_no[k]
      } else{
        spend_no[k] <- spend_no[k - 1] + spend_no[k]
      }
    }
    for(k in 1:n){
        spend_no[k] <- spend_no[k]/(k + 1)
    }
...
simulation2(noShocks, 65, .7, 1)
simulation2(heartAttack, 65, .7, 1)
simulation2(cancer, 65, .7, 1)
